...
def render(prs: list[dict], username: str) -> str:
    ...
    if not prs:
        lines.extend(["_No merged external pull requests found yet — the next quest is loading._", ""])
    else:
        for repo, items in list(groups.items())[:MAX_REPOS]:
            repo_url = f"https://github.com/{repo}"
            lines.extend([f"### [{repo}]({repo_url}) · {len(items)} merged", ""])
    ...
    lines.append(f"[View all merged pull requests →](https://github.com/pulls?q={query})")
    return "\n".join(lines)
...
def main() -> int:
    ...
    updated = replace_section(current, render(fetch_merged_prs(username, token), username))
    if updated != current:
